...
define('RAVEPAY_LOGGING_LEVEL', 4);
define( 'CCRM_RAVE_PLUGIN_FILE', __FILE__ );
// Number of failures at which to cancel a recurring payment
define('RAVEPAY_CANCEL_RECURRING_ON_N_FAILURES', 10);

require_once 'CRM/Core/Payment.php';

class flutterwave_payment_ravepay extends CRM_Core_Payment {
    ...
    public static function &singleton($mode = 'test', &$paymentProcessor, &$paymentForm = null, $force = false) {
        ...
        if (is_null(self::$_singleton[$processorName]))
            self::$_singleton[$processorName] = new flutterwave_payment_ravepay($mode, $paymentProcessor);
    ...
    public function __construct($mode, &$paymentProcessor) {
        ...
        $this->_processorName    = ts('Ravepay');
        $this->logging_level     = RAVEPAY_LOGGING_LEVEL;
        ...
        switch ($action) {
            ...
            case 'get':
                if ($data = CRM_Core_DAO::singleValueQuery("
                   SELECT data FROM civicrm_ravepay WHERE data_type = %1 AND entity_id = %2    
                ", array(
           ...
           case 'update':
                CRM_Core_DAO::executeQuery("
                   UPDATE civicrm_ravepay SET data = %1 WHERE data_type = %2 AND entity_id = %3
                ", array(
           ...
           case 'delete':
                CRM_Core_DAO::executeQuery("
                   DELETE FROM civicrm_ravepay WHERE data_type = %1 AND entity_id = %2
                ", array(
                      1 => array($data_type, 'String'),
                      2 => array($params['entity_id'], 'Integer')
                   )
    ...
    public function checkConfig() {
        ...
        if (!$this->_paymentProcessor['user_name']){
            $errors[] = 'No live public key set for Rave payment gateway';
        }
        if (!$this->_paymentProcessor['password']){
            $errors[] = 'No live secret key set for Rave payment gateway';
        }

        if (!empty($errors))
            // return '<p>' . implode('</p> <p>', $errors) . '</p>';
            return 'Live secret or public key is not set for Rave payment gateway';
    ...
    public function doRepeatCheckout(&$params) {
        ...
        $ipn = new flutterwave_payment_ravepay_notify($this);
    ...
    public function doTransferCheckout(&$params, $component = 'contribute') {
        ...
        // brief to avoid this
        $notifyParams = array(
            'processor_name' => 'Rave by Flutterwave',
        ...
        if ($use_billing = CRM_Core_BAO_Setting::getItem('uk.co.circleinteractive.payment.ravepay', 'use_billing')) {
        ...
        // Construct params list to send to Ravepay ..
        ...
        $reference = $params['contributionID']. '_'.$params['invoiceID'];
        $payment_array = array(
            'PBFPubKey' => $this->_paymentProcessor['user_name'],
            'customer_email' => $contact['email'],
            'amount' => sprintf("%.2f", $params['amount']),
            'currency' => $params['currencyID'],
            'payment_method' => 'both',
            'txref' => $reference,
            'redirect_url' => $notifyURL.'&reference='.$reference,
            'hosted_payment'=> 1
        );
        ...
        echo '<script type="text/javascript" src="'.$url.'/flwv3-pug/getpaidx/api/flwpbf-inline.js"></script>';
        echo '<script>';
    ...
    public function error($message, $function=null, $lineno=null) {
        ...
        if (function_exists('watchdog'))
            watchdog('civicrm_ravepay', $message, array(), WATCHDOG_ERROR);
    ...
    public function handlePaymentCron() {
        ...
        // Retrieve all recurring payments for Ravepay that are 'In progress' and have a payment due
    ...
    public function install() {
         ...
         require_once "CRM/Core/DAO.php";
         CRM_Core_DAO::executeQuery("
            CREATE TABLE IF NOT EXISTS `civicrm_ravepay` (
            ...
            if (self::getCRMVersion() < 4.3) {
                CRM_Core_DAO::executeQuery("
                    INSERT INTO civicrm_job (
                       ...
                       NULL, %1, 'Daily', NULL, 'Process Ravepay Recurring Payments', 
                       'Processes any Ravepay recurring payments that are due',
                       'civicrm_api3', 'job', 'run_payment_cron', 'processor_name=Ravepay', 0
                    )
                    ", array(
                ...
                $result = civicrm_api('job', 'create', array(
                    'version'       => 3,
                    'name'          => ts('Process Ravepay Recurring Payments'),
                    'description'   => ts('Processes any Ravepay recurring payments that are due'),
                    'run_frequency' => 'Daily',
                    'api_entity'    => 'job',
                    'api_action'    => 'run_payment_cron',
                    'parameters'    => 'processor_name=Ravepay',
                    'is_active'     => 0
                ));
    ...
    public function log($message, $level=4) {
        if ($this->logging_level >= $level) {
            ...
            if (function_exists('watchdog'))
                watchdog('civicrm_ravepay', $message, array(), WATCHDOG_INFO);
        }
    }

    public function uninstall() {

        // On uninstall, remove ravepay data table
        require_once "CRM/Core/DAO.php";

        // probably not a good idea to remove this table once created - andyw, 22/07/2014
        //CRM_Core_DAO::executeQuery("DROP TABLE civicrm_ravepay");
    ...
    public function updateFailureCount($contribution_recur_id) {
        ...
        ) >= RAVEPAY_CANCEL_RECURRING_ON_N_FAILURES) {
    ...
    protected function getIDs() {
        ...
        $dao = CRM_Core_DAO::executeQuery("
            SELECT id FROM civicrm_payment_processor 
             WHERE class_name = 'uk.co.circleinteractive.payment.ravepay'
        ");
        while ($dao->fetch())
            $ids[] = $dao->id;
        return $ids;
    ...
    protected function requestPost($url, $data){
        ...
        if (!function_exists('curl_init'))
            CRM_Core_Error::fatal(ts('CiviCRM Ravepay extension requires the component \'php5-curl\'.'));
     ...
     * 'flutterwave_payment_ravepay_notify' when redirecting.
     *
     * @custom
